from emoji_mapping import get_emoji_mapping
emoji_mapping = get_emoji_mapping()
def do_emoji_mapping(text):
    for key in emoji_mapping.keys():
        text = text.replace(key, emoji_mapping[key])
    return text

import re
def processed_model_outputs(result, input_text):
    processed = []
    for text in result:
        text = text.split("\n\n")[0]
        text = text.replace(input_text, "")
        text = do_emoji_mapping(text)
        text = re.sub(r"\[.*?\]", "", text)
...
if True:
    ...
    args.topk = 8
    args.topp = 0
    args.model_config = 'config/model_config_small.json'
    args.tokenizer_path = 'cache/vocab_small.txt'
    args.model_path = 'model/final_model'
...
def model_generate_text(input_text, nsamples, model = model, args = args):
    length = args.length
    batch_size = args.batch_size
    #nsamples = args.nsamples
    temperature = args.temperature
